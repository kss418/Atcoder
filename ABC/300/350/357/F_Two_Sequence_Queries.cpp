...
public:
    ...
    public:
        ll ab, a, b, len;
        node() : node(0, 0, 0, 0){}
        node(ll ab, ll a, ll b, ll len) : ab(ab), a(a), b(b), len(len){}
    ...
    public:
        ll a, b;
        lazy_type() : lazy_type(0, 0){}
        lazy_type(ll a, ll b) : a(a), b(b){}
    ...
    node merge(node l, node r){ 
        return { (l.ab + r.ab) % MOD, (l.a + r.a) % MOD, (l.b + r.b) % MOD, l.len + r.len };
    }
    ...
    void cal(lazy_type lazy, node& seg){ //lazy로 인한 seg 값 계산
        seg.ab += (lazy.b * seg.a + lazy.a * seg.b) % MOD + seg.len * ((lazy.a * lazy.b) % MOD);
        seg.a += lazy.a * seg.len;
        seg.b += lazy.b * seg.len;

        seg.ab %= MOD; seg.a %= MOD; seg.b %= MOD;
    }
...
void run(){
    cin >> n >> m; _prop seg(n + 1);
    for(int i = 1;i <= n;i++) cin >> a[i], a[i] %= MOD;
    for(int i = 1;i <= n;i++) cin >> b[i], b[i] %= MOD;
    for(int i = 1;i <= n;i++) seg.set(i, {a[i] * b[i] % MOD, a[i], b[i], 1});
    seg.init();
