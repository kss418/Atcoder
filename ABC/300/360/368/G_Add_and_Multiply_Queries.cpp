...
void run(){
    cin >> n; num.insert(INF); _seg seg(n + 1);
    for(int i = 1;i <= n;i++) cin >> a[i], seg.update(i, a[i]);
    for(int i = 1;i <= n;i++){
        cin >> b[i];
        if(b[i] != 1) num.insert(i);
    }
    ...
    while(m--){
        ...
        if(op == 1){
            ...
            if(b[idx] != 1) num.insert(idx);
            else if(num.count(idx)) num.erase(num.find(idx));
        }
        else{
            ll l, r; cin >> l >> r;
            ll now = a[l];
            ...
            while(idx <= r){
                if(b[idx] == 1){
                    ll nxt = *num.upper_bound(idx);
                    if(nxt == INF || nxt > r) now += seg.query(idx, r);
                    else now += seg.query(idx, nxt - 1);

                    idx = nxt;
                }
                else{
                    ll nxt = max(now * b[idx], now + a[idx]);
                    now = nxt; idx++;
                }
            }
