...
public:
    ...
    public:
        ll v;
        lazy() : lazy(-1){} // identity
        lazy(ll v) : v(v){}
        bool is_id(){ return v == -1; }
    }; vector <lazy> lz;
...
void run(){
    ...
    for(int i = 1;i <= n;i++){
        ...
        if(op == 1){
            ll v; cin >> v;
            adj[v].push_front(i);
            query.push_back({1, v, i});
        }   
        else{
            ll l, r; cin >> l >> r;
            query.push_back({2, l, r});
        }
    }

    dfs();
    for(auto& [op, l, r] : query){
        if(op == 1) seg.update(org[r], r);
        else{
            l = org[l]; r = org[r];
            if(l > r) swap(l, r);
            ll now = seg.query(l + 1, r - 1);
